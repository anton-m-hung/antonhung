function [X,yFitted,u,ue,r]=cf222h(X1,Y,beta,range,zcf,zr,n)
%Fits a nonlinear curve of the form Y=A*exp(-uX)*h(X) where beta is an
%approximation of this exponent and h(X) is the confocal function.
...
modelfun = @(b,X)b(1)*exp(-b(2)*X)./((X-zcf).^2/(2*zr*n)^2+1); %Sample function
...
yFitted = coefficients(1) *exp(-u*X)./((X-zcf).^2/(2*zr*n)^2+1);

function [X,yFitted,u,ue,r]=LogSimpleBeer_fitted(X1,Y,beta,range)
%Fits a nonlinear curve of the form Y=A*exp(-uX) where beta is an
%approximation of this exponent.
...
beta0=[1 beta];                        %Initial guesses for the amplitude-coefficient                         
%[amplitude    lamda]  %Reference array for best guess
resolution = 5; % number microns used to generate a fit to calculate scattering coefficent 

modelfun = @(b,X) b(1)*exp(-b(2)*X); %Sample function
...
yFitted = coefficients(1)* exp(-u*X);

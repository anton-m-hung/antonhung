...
filter_mod <- callModule(
  ...
  id = "study-filters",
  data = study,
  vars = c("madi_program", "research_focus", "condition_preferred", "type", "measurement_technique", "species", "sex", "clinical_trial", "have_assessment", "have_test", "actual_enrollment", "sponsoring_organization", "study_accession", "tree_id")
)
  ...
  if(is.null(input$experiment_radio)){
    ...
    output$experiment_download <- renderUI({
      fluidRow(actionButton("download_excel_check", "Download experiment(s)")
               , actionButton("launchRadiant", label="Launch the MADI explorer toolset")
    ...
    output$experiment_download <- renderUI({
      fluidRow(actionButton("download_excel_check", "Download Experiment(s)")
               , actionButton("launchRadiant", label="Launch MADI explorer toolset")
#               , actionButton("transformSet", label="Transform data")
               , fileInput("upload_experiment", "Upload Experiment", multiple = FALSE, accept = ".xlsx")
               )
    })

my.Intralipid.concentrations = c(1,4,13,20)
my.attenuation.coefficient = c(0.407896, 1.2788, 1.81108, 1.94128)
my.attenuation.coefficient = my.attenuation.coefficient * 1.353

Intralipid.concentrations = c(1,5,10,20)
Attenuation.coefficient = c(0.6203, 3.2202, 6.4088, 8.0320)

# plot(Intralipid.concentrations, Attenuation.coefficient, type="o", col='blue')
plot(my.Intralipid.concentrations, my.attenuation.coefficient, type="o", col='blue')

lines(my.Intralipid.concentrations, my.attenuation.coefficient, col=2)
points(my.Intralipid.concentrations, my.attenuation.coefficient, col=2)

legend(1, 8, legend=c("My values", "Literature values"),
     col=c("red", "blue"), lty=1:1, cex=0.7)

setwd("/users/anton/desktop/McCord_Mermutlab/assignments/FINALTHESIS/results")
my.intralipid.table <- read.csv("./intralipid_results.csv", sep=";")

my.intralipid.table <- my.intralipid.table[,1:2]

my.intralipid.df <- as.data.frame(my.intralipid.table)
colnames(my.intralipid.df) <- c("Intralipid concentration (%)", "Attenuation coefficient (mm^-1)")
my.intralipid.df$`Intralipid concentration (%)` = as.factor(my.intralipid.df$`Intralipid concentration (%)`)
colours <- c("grey80", "grey50", "grey20", "black")
levels(my.intralipid.df$`Intralipid concentration (%)`)

install.packages("RColorBrewer")
library(RColorBrewer)

library(tidyverse)
my.plot <- ggplot(data=my.intralipid.df,  aes(x=fct_inorder(my.intralipid.df[,1]), y=my.intralipid.df[,2], colour=`Intralipid concentration (%)`)) +
